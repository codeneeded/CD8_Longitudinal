...
#   v5 — Updated for 10 αβ CD8 populations, fixed ADT averaging, fixed paths
#
# Changes from v4:
#   - Paths: analysis reads from Analysis/CD8_subset, output to Manuscript/Supplementary 4
#   - 13 → 10 populations (no γδ/MAIT)
#   - ADT averaging: manual rowMeans (no AverageExpression expm1 bug)
#   - S4 heatmaps: γδ/MAIT markers removed, "10 αβ CD8" in titles
#   - S4C: Naïve CD8 4 added to gating
#   - S6: CD16+ Effector volcano added
#   - S7: Naïve CD8 4 pairwise comparisons added
#   - effector_clusters: TEM, TEMRA, CD16+ Effector
################################################################################

library(Seurat); library(ggplot2); library(dplyr); library(tidyr)
library(cowplot); library(pheatmap); library(grid); library(qs2)
library(ggrepel); library(ggpubr); library(SeuratExtend); library(patchwork)

# ── Paths ─────────────────────────────────────────────────────────────────────
base_dir     <- "~/Documents/CD8_Longitudinal"
saved_dir    <- file.path(base_dir, "saved_R_data")
analysis_dir <- file.path(base_dir, "Analysis", "CD8_subset")
csv_dir      <- file.path(analysis_dir, "CSV_outputs")

s4_dir  <- file.path(base_dir, "Manuscript", "Supplementary 4")
s5_dir  <- file.path(base_dir, "Manuscript", "Supplementary 5")
s6_dir  <- file.path(base_dir, "Manuscript", "Supplementary 6")
s7_dir  <- file.path(base_dir, "Manuscript", "Supplementary 7")
s8_dir  <- file.path(base_dir, "Manuscript", "Supplementary 8")
s9_dir  <- file.path(base_dir, "Manuscript", "Supplementary 9")
...
s4_group_colors <- c(
  "Naive/Quiescence"="#74C2E1","Memory/Survival"="#66BB6A",
  "Co-stimulation/Homing"="#AED581","Effector/Cytotoxicity"="#EF5350",
  "Terminal Diff."="#AB47BC","Exhaustion"="#A1887F",
  "Activation"="#FFD54F","Innate-like/NK"="#F06292",
  "Tissue Residency"="#9C755F","TCR Identity"="#9C6FD6"
)
...
# ── S4A: ADT heatmap (FIXED: manual rowMeans, no log10) ─────────────────────
cat("  S4A: ADT heatmap...\n")
...
adt_heatmap_markers <- c(
  ...
  "ENTPD1"="Exhaustion","NT5E"="Exhaustion",
  "CD38"="Activation","CD69"="Activation","FAS"="Activation",
  "CXCR3"="Tissue Residency","ITGA1"="Tissue Residency",
...
# FIXED: manual rowMeans instead of AverageExpression (avoids expm1 bug)
adt_raw <- GetAssayData(TARA_cd8, slot = "data")[adt_feats, , drop = FALSE]
clusters_vec <- TARA_cd8$CD8_Annotation
avg_adt <- sapply(levels(factor(clusters_vec)), function(cl) {
  ...
  rowMeans(adt_raw[, cells, drop = FALSE])
})
rownames(avg_adt) <- adt_feats

# FIXED: no log10 — DSB values are already on meaningful scale
avg_adt_s <- scale_01(as.matrix(avg_adt))
...
p_adt <- pheatmap(
  ...
  main = "Surface protein (ADT) expression — 10 αβ CD8 sub-clusters",
  silent = TRUE
)

png(file.path(s4_dir, "S4A_ADT_heatmap.png"), width = 26, height = 18, units = "in", res = 300, bg = "white")
grid.draw(p_adt$gtable)
dev.off()

write.csv(as.data.frame(adt_mat), file.path(csv_dir, "S4A_ADT_heatmap_scaled_values.csv"))

# ── S4B: RNA heatmap (UPDATED: removed γδ/MAIT markers) ─────────────────────
cat("  S4B: RNA heatmap...\n")
...
rna_heatmap_markers <- c(
  ...
  "HLA-DRA"="Activation","CD38"="Activation","MKI67"="Activation",
  ...
  "GZMK"="Effector/Cytotoxicity",
  "TBX21"="Terminal Diff.","ZEB2"="Terminal Diff.",
  "PRDM1"="Terminal Diff.","CX3CR1"="Terminal Diff.","EOMES"="Terminal Diff.",
  "TOX"="Exhaustion","PDCD1"="Exhaustion","TIGIT"="Exhaustion",
  "HAVCR2"="Exhaustion","LAG3"="Exhaustion","CXCL13"="Exhaustion",
  "NCAM1"="Innate-like/NK","FCGR3A"="Innate-like/NK",
  "KLRD1"="Innate-like/NK","TYROBP"="Innate-like/NK"
)
...
p_rna <- pheatmap(
  ...
  main = "mRNA expression — 10 αβ CD8 sub-clusters",
  silent = TRUE
)
...
# ── S4C: ADT gating (UPDATED: includes Naïve CD8 4) ─────────────────────────
cat("  S4C: ADT gating...\n")
...
if (sum(naive_mask) > 0) {
  ...
  if ("source_cluster" %in% colnames(TARA_cd8@meta.data)) {
    ...
    gate_plot_df$facet <- paste0("Source cluster ", gate_plot_df$source)
  } else {
    gate_plot_df$facet <- "All naïve clusters"
  }
  
  gate_cols <- c("Tscm CD8" = "#E31A1C", "Naïve Intermediate CD8" = "#1F78B4")
...
png(file.path(s5_dir, "S5A_composition_by_ART.png"), width = 14, height = 14, units = "in", res = 300, bg = "white")
...
# S5B: Distribution of each condition across clusters (column-normalized)
...
png(file.path(s5_dir, "S5B_condition_distribution.png"), width = 14, height = 14, units = "in", res = 300, bg = "white")
...
################################################################################
# S6: EFFECTOR DGE — TEM + TEMRA + CD16+ Effector volcanos
################################################################################
cat("\n=== S6: Effector DGE ===\n")

# UPDATED: 3 effector volcanos (added CD16+ Effector)
volc_files <- list(
  ...
  list(label="S6C", title="CD16+ Effector CD8", file="DGE_MAST_Expanding_CD16EffectorCD8_Sup_vs_Unsup.csv")
)

for (vf in volc_files) {
  fpath <- file.path(analysis_dir, "03_DGE", vf$file)
  if (!file.exists(fpath)) { cat("    SKIP", vf$label, "— file not found\n"); next }
  ...
  p <- ggplot(dge, aes(x = pmax(pmin(avg_log2FC, x_lim*0.98), -x_lim*0.98),
    ...
    geom_label_repel(data = dge %>% filter(label != ""),
                     ...
                     segment.size = 0.4, box.padding = 0.5, label.size = 0.2, show.legend = FALSE) +
  ...
  ggsave(file.path(s6_dir, paste0(vf$label, "_Volcano_", gsub("[^A-Za-z0-9]", "_", vf$title), ".png")),
         plot = p, width = 14, height = 11, dpi = 300, bg = "white")
  file.copy(fpath, file.path(csv_dir, paste0(vf$label, "_DGE_", gsub("[^A-Za-z0-9]", "_", vf$title), ".csv")),
            overwrite = TRUE)
}
cat("  S6 done.\n")
...
################################################################################
# S7: NAÏVE PAIRWISE VOLCANOS (UPDATED: includes Naïve CD8 4)
################################################################################
...
# UPDATED: all pairwise among original 3 naïve + new Naïve CD8 4
# DGE file names use sanitized cluster names from the pipeline
naive_comps <- list(
  list(title="Naïve CD8 vs Naïve CD8 2", id1="Naïve CD8", id2="Naïve CD8 2",
       file="DGE_MAST_RNA_NaveCD8_vs_NaveCD82.csv"),
  list(title="Naïve CD8 vs Naïve CD8 3", id1="Naïve CD8", id2="Naïve CD8 3",
       file="DGE_MAST_RNA_NaveCD8_vs_NaveCD83.csv"),
  list(title="Naïve CD8 vs Naïve CD8 4", id1="Naïve CD8", id2="Naïve CD8 4",
       file="DGE_MAST_RNA_NaveCD8_vs_NaveCD84.csv"),
  list(title="Naïve CD8 2 vs Naïve CD8 3", id1="Naïve CD8 2", id2="Naïve CD8 3",
       file="DGE_MAST_RNA_NaveCD82_vs_NaveCD83.csv"),
  list(title="Naïve CD8 2 vs Naïve CD8 4", id1="Naïve CD8 2", id2="Naïve CD8 4",
       file="DGE_MAST_RNA_NaveCD82_vs_NaveCD84.csv"),
  list(title="Naïve CD8 3 vs Naïve CD8 4", id1="Naïve CD8 3", id2="Naïve CD8 4",
       file="DGE_MAST_RNA_NaveCD83_vs_NaveCD84.csv")
)

for (i in seq_along(naive_comps)) {
  nc <- naive_comps[[i]]
  label <- paste0("S7_", LETTERS[i])
  fpath <- file.path(analysis_dir, "03_DGE", nc$file)
  if (!file.exists(fpath)) { cat("    SKIP", label, ":", nc$file, "\n"); next }
  cat("    ", label, ":", nc$title, "...\n")
  ...
  ggsave(file.path(s7_dir, paste0(label, "_Volcano.png")),
         plot = p, width = 14, height = 11, dpi = 300, bg = "white")
  file.copy(fpath, file.path(csv_dir, paste0(label, "_DGE.csv")), overwrite = TRUE)
}
...
p_s7_legend_src <- ggplot(data.frame(x=1:5, y=1:5,
  ...
  scale_color_manual(values=naive_cols, name="Gene category", drop=FALSE) +
  theme_void() +
  theme(legend.direction="horizontal", legend.text=element_text(size=22),
...
ggsave(file.path(s7_dir, "S7_common_legend.png"),
       plot = as_ggplot(cowplot::get_legend(p_s7_legend_src)),
       width = 20, height = 2.5, dpi = 300, bg = "white")

cat("  S7 done.\n")


################################################################################
# S8: PSEUDOTIME MODULES + STEMNESS VS VL
#   UPDATED: IFN memory moved to main Fig 5D, replaced by Stemness
#   Added: Stemness vs VL scatter (moved from Fig 5F)
################################################################################
cat("\n=== S8: Pseudotime modules + stemness ===\n")
...
if (file.exists(m3_pt_path) & file.exists(module_path)) {
  ...
  # UPDATED: Cytotoxicity, Inflammatory_Chemokines, Stemness (not IFN memory)
  for (m in c("Cytotoxicity", "Inflammatory_Chemokines", "Stemness")) {
    if (m %in% colnames(mod_scores)) pt_mod[[m]] <- mod_scores[common, m]
  }
  ...
  s8_panels <- list(
    ...
    list(col="Stemness", ylab="Stemness score",
  ...
  for (i in seq_along(s8_panels)) {
    sp <- s8_panels[[i]]
    if (!sp$col %in% colnames(pt_mod)) next
    s8_plot_list[[i]] <- ggplot(pt_mod, aes(x = pseudotime, y = .data[[sp$col]], color = Timepoint)) +
  ...
  s8_legend <- cowplot::get_legend(
    ggplot(pt_mod, aes(x=pseudotime, y=Cytotoxicity, color=Timepoint)) +
      geom_point(size=5) +
  ...
  if (length(s8_plot_list) == 3) {
    combined_s8_row1 <- (s8_plot_list[[1]] | s8_plot_list[[2]] | s8_plot_list[[3]]) /
      wrap_elements(full = s8_legend) + plot_layout(heights = c(1, 0.08))
    ggsave(file.path(s8_dir, "S8_Row1_pseudotime_modules.png"),
           plot = combined_s8_row1, width = 27, height = 8, dpi = 300, bg = "white")
  }
  
  write.csv(pt_mod, file.path(csv_dir, "S8_pseudotime_module_scores.csv"), row.names = FALSE)
  
  # Row 2: Stemness vs VL scatter (moved from main Fig 5F)
  if (file.exists(vl_path)) {
    ...
    p_s8_stem_vl <- ggplot(sample_scores, aes(x = log10_VL, y = Stemness)) +
    ...
    ggsave(file.path(s8_dir, "S8_Stemness_vs_VL.png"),
           plot = p_s8_stem_vl, width = 9, height = 8, dpi = 300, bg = "white")
  }
}
cat("  S8 done.\n")
...
if ("KIR+ innate-like CD8" %in% unique(TARA_cd8$CD8_Annotation)) {
  ...
  # S9C: Volcano
  cat("    S9C: KIR+ volcano...\n")
  kir_dge_path <- file.path(analysis_dir, "03_DGE", "DGE_MAST_Expanding_KIRinnateLikeCD8_Sup_vs_Unsup.csv")
  ...
  if (!file.exists(kir_dge_path)) {
    ...
    if (min(table(kir_post$Timepoint_Group)) >= 10) {
      ...
      write.csv(kir_dge, kir_dge_path)
    }
  }
  ...
  if (file.exists(kir_dge_path)) {
    ...
    if (nrow(kir_dge) > 0) {
      ...
      p_s9c <- ggplot(kir_dge, aes(x = pmax(pmin(avg_log2FC, kx*0.98), -kx*0.98),
                                   y = pmin(neg_log10_padj, ky), color = highlight)) +
  ...
  for (candidate in c("CTaa","CTstrict","CTgene","CTnt","clonotype_id")) {
    if (candidate %in% colnames(kir_cells@meta.data)) { tcr_col <- candidate; break }
  }
...
cat("Figures: Manuscript/Supplementary 4 through 9\n")
cat("CSVs:   ", csv_dir, "\n")