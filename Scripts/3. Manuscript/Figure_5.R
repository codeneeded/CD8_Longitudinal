...
if (file.exists(m3_pt_path)) {
  ...
  # UPDATED: trajectory paths for new annotation
  # Naïve trajectory: Naïve CD8 → Naïve CD8 2 and → Naïve CD8 3
  # Effector trajectory: Naïve CD8 → Naïve CD8 4 → TEM CD8 → TEMRA CD8 → CD16+ Effector CD8
  path1 <- make_path_df(cluster_centroids,
                        c("Naïve CD8", "Naïve CD8 2"), "Naïve trajectory 1")
  path2 <- make_path_df(cluster_centroids,
                        c("Naïve CD8", "Naïve CD8 3"), "Naïve trajectory 2")
  ...
  cat("    WARNING: Monocle3 pseudotime file not found at", m3_pt_path, "\n")
}
...
################################################################################
cat("  Panels E, F: VL correlations...\n")
...
if (file.exists(vl_path)) {
  ...
  p_vl_legend <- ggplot(sample_scores, aes(x = log10_VL, y = Exhaustion)) +
    ...
    guides(color = guide_legend(override.aes = list(size = 5), nrow = 1, order = 1),
  ...
  row3 <- (p_5E | p_5F) / as_ggplot(legend_row3) + plot_layout(heights = c(1, 0.12))
